...
function get_admin_user($user_id){
		...
		$menu='';
		$menu.='<a href="admin_staff.php" style="color: #AA3E03;"><div class="home-btn">Home</div></a>';
...
//		$menu.='<a href="staff.php" style="color: #AA3E03;"><div class="home-btn">Home</div></a>';
		...
		$all_categories=get_all_categories();
			...
			$all_news=get_news_for_category($category['id'],false);//vrati sve vijesti za trenutnu kategoriju
				...
				$menu.='<a input type="submit" class="category_view btn" href="admin_create_news.php?id='.$category['id'].'">'.$category['title'].'<br></a>';
	...
	$query="SELECT * FROM news WHERE category_id='1' ORDER BY datetime DESC";
	$result=mysqli_query($conn,$query);
	confirm_query($result);
	...
	$query="SELECT * FROM news WHERE category_id='2' ORDER BY datetime DESC";
	$result=mysqli_query($conn,$query);
	confirm_query($result);
	...
	$query="SELECT * FROM news WHERE category_id='3' ORDER BY datetime DESC";
	$result=mysqli_query($conn,$query);
	confirm_query($result);
	...
	$query="SELECT * FROM news WHERE category_id='4' ORDER BY datetime DESC";
	$result=mysqli_query($conn,$query);
	confirm_query($result);
	...
	$query="SELECT * FROM news WHERE category_id='56' ORDER BY datetime DESC";
	$result=mysqli_query($conn,$query);
	confirm_query($result);
	...
	$query="SELECT * FROM news WHERE category_id='9' ORDER BY datetime DESC";
	$result=mysqli_query($conn,$query);
	confirm_query($result);
	...
	$query="SELECT * FROM news WHERE category_id='55' ORDER BY datetime DESC";
	$result=mysqli_query($conn,$query);
	confirm_query($result);
...
//                     
                    ...
                    echo '<div id="usr_img"><img src="'.$user['image'].'" data-scalemode="width" data-nativeanimation="true" class="img-responsive img-circle"></div>';   
                }
        }
    }
}
...
//           
        ...
        function show_admin_listings(){
            ...
            $query = "SELECT * FROM news WHERE admin_id='$user_id'";
            $result = mysqli_query($conn,$query);
            confirm_query($result);
		...
		$output.='<td>Delete</td>';
          ...
          while($news=mysqli_fetch_assoc($result)){
                $output.='<tr>';
		$output.='<td><a href="admin_content.php?id='.$news['id'].'">'.$news['title'].'</a></td>';
		$output.='<td><a href="admin_content.php?id='.$news['id'].'">'.($news['img_src']=='' ? 'No image': '<img src="'.$news['img_src'].'" width="150">').'</a></td>';
		$output.='<td>'.($news['visible']==0 ? 'No':'Yes').'</td>';
		$output.='<td>'.$news['datetime'].'</td>';
		$output.='<td><a href="admin_edit_news.php?id='.$news['id'].'"><span class="glyphicon glyphicon-pencil text-primary"></span></a></td>';
		$output.='<td><a href="delete_admin_news.php?id='.$news['id'].'"><span class="glyphicon glyphicon-remove text-danger"></span></a></td>';
                $output.='</tr>';    }	
                $output.='</table>';
           echo '<br><br>';
